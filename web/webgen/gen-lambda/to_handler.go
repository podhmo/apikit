...
func WriteHandlerFunc(w io.Writer,
	...
	// 	return fmt.Errorf("invalid path bindings, routing=%v, args=%v (in %s)", info.VarNames, pathBindings, def.Symbol)
	// }
	if len(dataBindings) > 1 {
		return fmt.Errorf("invalid data bindings, support only 1 struct, but found %d (in %s)", len(dataBindings), def.Symbol)
	}
